...
export default function CompanyTasksPage() {
  ...
  const renewalTypeFilter = Array.isArray(params.renewalType) ? params.renewalType[0] : params.renewalType;
      ...
      <div>
        {tasksLoading ? (
            ...
            <div className="space-y-8">
              ...
              {/* Show tasks for the specific renewal type only */}
              {allTasksCount > 0 && (() => {
                const renewalLabel = policyTypes.find(p => p.value === renewalTypeFilter)?.label || renewalTypeFilter;
                const matchingRenewal = upcomingRenewals.find(r => r.type === renewalTypeFilter);

                // Group tasks by status
                const statusGroups = Object.values(tasksByStatus).flat().reduce((groups: { [key: string]: CompanyTask[] }, task) => {
              ...
              {/* Show section if no tasks exist yet but there's an upcoming renewal for this specific type */}
              {allTasksCount === 0 && upcomingRenewals.filter(renewal => renewal.type === renewalTypeFilter).map(renewal => {
