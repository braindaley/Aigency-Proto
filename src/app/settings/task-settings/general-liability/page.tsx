...
export default function GeneralLiabilityTasksPage() {
  ...
  const fetchTasks = async () => {
    ...
    try {
      ...
      const q = query(tasksCollection, where('policyType', '==', 'general-liability'));
        ...
        {tasks.map((task) => (
          ...
          >
            ...
            <Button asChild variant="outline" size="sm">
              <Link href={`/settings/task-settings/general-liability/${task.id}`}>
                View
              </Link>
            </Button>
          </li>
  ...
  return (
    <div className="mx-auto max-w-screen-lg px-4 py-8 md:py-12">
      ...
      <div className="flex justify-between items-center mb-2">
        <div>
            <h1 className="text-3xl font-bold">Tasks: General Liability</h1>
            <p className="text-muted-foreground mt-2">Manage global tasks for General Liability policies.</p>
        </div>
        <Button asChild>
          <Link href="/settings/task-settings/new?policyType=general-liability">
